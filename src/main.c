...
void update_camera_matrix(ecs_iter_t* it) {
	...
	for (int i = 0; i < it->count; i++)
	{
		...
		mat4x4_scale(data->view, data->view, trans[i].scale);
	}
}
...
int main()
{
	...
	ECS_SYSTEM(world, update_camera_matrix, EcsOnUpdate, CameraViewProj, CameraSetting, LocalTransfrom);
	logger_init();
	random_init();
	...
	LocalTransfrom transform = transform_default();
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		draw_renderer(&transform, &quad_render, &cam_view_proj->view, &cam_view_proj->proj, modelLoc);
